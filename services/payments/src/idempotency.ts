/**
 * Idempotency for PaySuite checkout (scope billing:paysuite:checkout).
 */
...
export const PAYSUITE_IDEMPOTENCY_SCOPE = IDEMPOTENCY_SCOPES.paysuiteCheckout;
...
export async function beginIdempotency(
  ...
  const scope = PAYSUITE_IDEMPOTENCY_SCOPE;
...
export async function completeIdempotency(
  ...
  await prisma.idempotencyRecord.update({
    where: {
      scope_key: { scope: PAYSUITE_IDEMPOTENCY_SCOPE, key },
    },
    data: {
      status: "COMPLETED",
      responseCode,
...
export async function failIdempotency(prisma: PrismaClient, key: string) {
  await prisma.idempotencyRecord.updateMany({
    where: {
      scope: PAYSUITE_IDEMPOTENCY_SCOPE,
      key,
      status: "STARTED",
    },
    data: { status: "FAILED" },
